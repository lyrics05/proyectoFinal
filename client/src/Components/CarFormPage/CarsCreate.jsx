...
const uploadImage = async () => {
  if (!imageSelected) {
    toast.error("Please select an image before uploading", { theme: "colored" });
    return;
  }

  setIsUploading(true);

  // Mostrar el toast de carga y guardar su ID
  const toastId = toast.loading("Uploading image...", { theme: "colored" });

  const formData = new FormData();
  formData.append("file", imageSelected);
  formData.append("upload_preset", "autosAplicacion");

  try {
    const response = await axios.post(
      "https://api.cloudinary.com/v1_1/dpknisukz/image/upload",
      formData
    );
    ...
    setCar((prevCar) => ({
      ...prevCar,
      img: imageUrl, // Asegurar que se guarda correctamente
    }));
    ...
    toast.update(toastId, {
      render: "Failed to upload image. Try again.",
      type: "error",
      isLoading: false,
      autoClose: 3000,
    });
